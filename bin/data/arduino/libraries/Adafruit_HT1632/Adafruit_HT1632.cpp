...
Adafruit_HT1632LEDMatrix::Adafruit_HT1632LEDMatrix(uint8_t data, uint8_t wr, Vector<int8_t> pins) : Adafruit_GFX(pins.size()*24, 16), matrices(NULL), matrixNum((uint8_t)pins.size()) {
  matrices = (Adafruit_HT1632 *)malloc(matrixNum * sizeof(Adafruit_HT1632));
	
	if(matrices) 
	{
		for(int8_t i=0; i<matrixNum; i++)
		{
			matrices[i] = Adafruit_HT1632(data, wr, pins[i]);
		}
	}
}
